...
import "@/app/css/PostEvent.css";
interface PostEventProps {
  post: Post;
//   recommendations: Recommendation[];
}
const PostCard: React.FC<PostEventProps> = (post) => {
    // const PostCard: React.FC<PostEventProps> = ({ post, recommendations }) => {
  const [comments, setComments] = useState<Recommendation[]>(() => {
    const storedComments = localStorage.getItem("comments");
    return storedComments ? JSON.parse(storedComments) : post.post.recommendations;
  });
  const [newComment, setNewComment] = useState<string>("");
  const [newRate, setNewRate] = useState<number>(0);
  const [images, setImages] = useState<string[]>(post.post.album || []);
  ...
  return (
    <div className="post-event">
      <div className="header">
        ...
        <p className="user-name">מאת {post.post.userName}</p>
        <p className="date">{post.post.createDate.toLocaleDateString()}</p>
      </div>
