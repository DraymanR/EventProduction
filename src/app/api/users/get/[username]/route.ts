...
//     }
// }
import { NextResponse, NextRequest } from 'next/server'; // וודא שאתה מייבא את NextRequest
import jwt, { JwtPayload } from 'jsonwebtoken';
import { UserModel, ConsumerModel, SupplierModel } from '@/app/lib/models/user';
import { User } from '@/app/types/user';
import connectDb from '@/app/lib/db/connectDb';
...
export async function GET(req: NextRequest) {  // השתמש ב-NextRequest
    try {
        await connectDb();
        ...
        // גישה לעוגיה
        const tokenCookie = req.cookies.get('token');  // tokenCookie הוא אובייקט מסוג RequestCookie
        const token = tokenCookie ? tokenCookie.value : null;  // גישה לערך של העוגיה
